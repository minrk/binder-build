...
var fetchTarball = function (source, dir, callback) {
  var branch = source.specified_branch ? source.specified_branch : source.default_branch
...
var generateName = function (source) {
  return source.specified_branch
    ? _processPath(source).split('/').join('-') + '-' + source.specified_branch
    : _processPath(source).split('/').join('-')
}

var generateDisplayName = function (source) {
  console.log('source', source)
  return source.specified_branch
    ? _processPath(source) + '/tree/' + source.specified_branch
    : _processPath(source)
}
